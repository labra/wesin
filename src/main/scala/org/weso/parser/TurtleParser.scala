...
trait TurtleParser extends Positional with RegexParsers {
  ...
  lazy val PN_CHARS_BASE_STR =
 	...
 	"\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD" + 
   	"\\x{10000}-\\x{EFFFF}]" 
 lazy val PN_CHARS_U_STR    = PN_CHARS_BASE_STR + "|_"
 lazy val PN_CHARS_STR		= PN_CHARS_U_STR + "|-|[0-9]|\\u00B7|[\\u0300-\u036F]|[\\u203F-\\u2040]" 
  
 lazy val PN_CHARS_BASE				= PN_CHARS_BASE_STR.r
 lazy val PN_CHARS_U				= PN_CHARS_U_STR.r
 lazy val PN_CHARS					= PN_CHARS_STR.r
 
 lazy val PN_PREFIX_STR				= PN_CHARS_BASE_STR + "((" + PN_CHARS_STR + "|\\." + ")*" + PN_CHARS_STR + ")?"
 lazy val PN_PREFIX					= PN_PREFIX_STR.r
